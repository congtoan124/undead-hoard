...
actor {
  ...
  private stable var name_ : Text = "Undead Hoard";
  private stable var symbol_ : Text = "UDH";
  private stable var desc_ : Text = "Undead Hoard NFT";
  ...
  public shared ({ caller }) func mintForMyself(payload : MintPayload) : async MintResult {
    let metadata : TokenMetadata = {
      attributes = [{
        key = "isPrivate";
        value = Bool.toText(payload.isPrivate);
      }];
    ...
    totalSupply_ += 1;
    let txid = addTxRecord(caller, #mint(?metadata), ?token.index, #user(blackhole), #user(caller), Time.now());
    return #Ok((token.index, txid));
  };
  ...
  public query ({ caller }) func getMyTokens() : async [TokenInfoExt] {
    let tokenIds = switch (users.get(caller)) {
      case (?user) {
  ...
  public query ({ caller }) func isTokenMine(tokenId : Nat) : async Bool {
    switch (_ownerOf(tokenId)) {
      case (?owner) {
        return owner == caller;
      };
